...
One orchestrator runs a bounded decide/observe/replan loop over a strict task-scoped tool registry. It records validated redacted arguments, outcomes, provenance IDs, stable errors, and timing. Recoverable observations include the failed tool and safe arguments so a provider can correct a call without hidden execution state. Production can explicitly select either the OpenAI Responses adapter or a native Ollama `/api/chat` adapter. Both validate the same strict Pydantic decision envelope and retain the same typed tool boundary. Ollama uses schema-constrained JSON, a loopback-only configured endpoint, bounded context/output, temperature zero, request timeouts, bounded retries, safe error mapping, and no credential. Demo mode remains a separate deterministic scripted path.

The product-specific live model evaluator remains above the provider boundary. Long local runs atomically checkpoint after every completed scenario, persist an evaluation-configuration fingerprint, label reports `PARTIAL` or `COMPLETE`, and resume only missing cases when provider, model, generation limits, timeout/retries, context, suite version, and case-set hash match exactly.
