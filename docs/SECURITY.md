...
This document tracks the threat model, implemented security controls, residual risks, and deployment limitations for V1.

Controls marked as implemented are enforced by the current codebase and covered by targeted tests where applicable. Items described as design requirements or limitations remain future work.

V1 is designed for single-user, local, or controlled deployment. It is not intended for sensitive multi-user production use because authentication, authorization, tenancy, retention policies, and several deployment-hardening controls remain out of scope.
...
- Implemented for structured datasets: 10 MiB upload, 100,000-row, and 200-column bounds are enforced across CSV, XLSX, JSON, Parquet, REST, and PostgreSQL imports. CSV/XLSX headers and PostgreSQL result metadata are checked before full row materialization where possible. General XLSX ingestion rejects excessive archive expansion, unsafe internal paths, macros, embedded objects, and external links before worksheet parsing.
...
- Formula-like text is neutralized in generated CSV/XLSX data and provenance sheets so uploaded or source-derived strings are not emitted as active spreadsheet formulas.
...
- Workflow-run snapshots contain source identities, content/configuration hashes, schema/type hashes, compact quality counts, secret-reference names where applicable, and artifact/fact references. They never resolve or store raw secret values. Definition fingerprints and immutable version rows prevent later recipe changes from rewriting the meaning of old runs. Run identifiers are append-only in both durable PostgreSQL and demo-mode repositories, so changed history cannot overwrite an existing run.
