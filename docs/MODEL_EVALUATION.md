...
| [`qwen3.5:4b`](https://ollama.com/library/qwen3.5:4b) (official artifact approximately 3.4 GB) | Not tested. | Identified as a smaller future candidate; no runtime claim has been made. | Not evaluated. Compatibility preflight and the unchanged 10-case benchmark are deferred. |

These results establish native provider execution and resumable evaluation, but they do not identify a recommended local default. Generated reports remain local and ignored: the reviewed conclusions above contain no prompts, response bodies, machine-specific paths, or secrets.
